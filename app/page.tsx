...
export default function Home() {
  ...
  // Datele Ticker-ului: Doar urgență maximă, sânge pe piață, fără emoji.
  const tickerItems = [
  ...
  return (
    <div className="flex flex-col w-full bg-white selection:bg-[#FFD100] selection:text-black font-sans">
        ...
        <div className="mx-auto max-w-7xl px-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 max-w-4xl mx-auto mb-16">
            {[
              ...
              <div key={item.label} className="p-6 border-2 border-black rounded-2xl bg-white shadow-[4px_4px_0_0_rgba(0,0,0,1)] hover:-translate-y-1 transition-all">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6">
              {categories.map((cat) => (
                <Link key={cat.slug} href={`/categorii/${cat.slug}`} className="group flex flex-col items-center gap-5">
                  <div className="w-20 h-20 md:w-24 md:h-24 bg-gray-50 rounded-3xl flex items-center justify-center border-2 border-gray-100 group-hover:border-black group-hover:bg-[#FFD100] group-hover:shadow-[6px_6px_0_0_rgba(0,0,0,1)] transition-all duration-300">
      ...
      {/* TICKER DE TRADING - Brutalist & Agresiv (Terminal Style) */}
        ...
        <div className="flex whitespace-nowrap animate-ticker items-center">
          {[...tickerItems, ...tickerItems, ...tickerItems].map((item, index) => (
            <span key={index} className="inline-flex items-center text-white font-black uppercase tracking-widest text-[11px] md:text-[13px] italic">
              {/* Eticheta roșie/galbenă ca un marker de alertă */}
      ...
      {/* SECȚIUNEA OPORTUNITĂȚI CASH */}
      <section className="py-24 bg-gray-50">
        <div className="mx-auto max-w-7xl px-4">
            <div className="flex justify-between items-end mb-16 border-b-4 border-black pb-8">
                <h2 className="text-5xl font-black uppercase italic tracking-tighter leading-none">Oportunități <span className="text-[#FFD100]">Cash</span></h2>
                <button className="text-xs font-black uppercase tracking-widest italic hover:text-[#FFD100] transition-colors">Vezi tot →</button>
            </div>
    
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12">
                <AdCard 
                    id="penthouse-phuket" 
                    title="Penthouse Panwa Bay" 
                ...
                <AdCard 
                    id="teren-buftea" 
                    title="Teren Buftea Lac" 
                    image="https://images.unsplash.com/photo-1500382017468-90499d631831?auto=format&fit=crop&w=800&q=80"
                    marketPrice="€220.000"
                    exitPrice="€165.000"
                    discount="25"
                    score={9.2}
